            ...
            for R in [''] if not with_erhua else ['', 'r']:
                for T in [''] if not with_tone else ['1', '2', '3', '4', '5']:
                    ...
                    if result:
                        ...
                        syllables[result] = re.sub('^ ', '', f'{C} {V}{R}{T}')
    return syllables
...
class G2pZh:
    ...
    def __init__(self):
        self.lexicon = generate_lexicon(True, True)
        self.symbols = generate_symbols(self.lexicon)
        self.model = G2pM()
        self.re = re.compile('u:')

    def character2phoneme(self, text):
        pinyin = ' '.join(self.model(text, tone=True, char_split=False))
        pinyin = self.re.sub('v',pinyin)
        phonemes = self.pinyin2phoneme(pinyin)
        return pinyin,' '.join(phonemes)

    def pinyin2phoneme(self, text):
        result = []
        for pinyin in text.split(' '):
            phoneme = self.lexicon.get(pinyin, pinyin)
            result.extend(phoneme.split(' '))
        return result

    def phn2id(self):
        return {phn: i + 1 for i, phn in enumerate(self.symbols)}
